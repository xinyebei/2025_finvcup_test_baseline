...
class Trainer(object):
    def __init__(
        ...
        if 'task_target' not in config:
            ...
            task_str = f"{config['task_target']}" if config['task_target'] is not None else ""
            self.log_dir = os.path.join(
                self.config['log_dir'],
                self.config['model_name'] + "_" + task_str + '_' + self.timenow
            )
    ...
    def train_step(self,data_dict):
        if self.config['optimizer']['type']=='sam':
            ...
            predictions = self.model(data_dict, inference=False)
    ...
    def train_epoch(
        ...
        for iteration, data_dict in tqdm(enumerate(train_data_loader),total=len(train_data_loader)):
            ...
            for key in data_dict.keys():
                if data_dict[key]!=None and key!='name' and isinstance(data_dict[key],torch.Tensor):
    ...
    def get_respect_acc(self,prob,label):
        ...
        acc_fake = np.count_nonzero(judge[zero_num:]) / (len(judge[zero_num:]) + 1)
        acc_real = np.count_nonzero(judge[:zero_num]) / (len(judge[:zero_num]) + 1)
    ...
    def test_one_dataset(self, data_loader):
        ...
        for i, data_dict in tqdm(enumerate(data_loader),total=len(data_loader)):
            ...
            for key in data_dict.keys():
                if data_dict[key]!=None and isinstance(data_dict[key],torch.Tensor):
                    data_dict[key]=data_dict[key].cuda()
    ...
    def save_best(self,epoch,iteration,step,losses_one_dataset_recorder,key,metric_one_dataset):
        ...
        for k, v in metric_one_dataset.items():
            if k == 'pred' or k == 'label' or k=='dataset_dict' or k == 'cls' or k == "img_names":
                continue
        ...
        if 'pred' in metric_one_dataset:
            acc_real, acc_fake = self.get_respect_acc(np.array(metric_one_dataset['pred']), np.array(metric_one_dataset['label']))
